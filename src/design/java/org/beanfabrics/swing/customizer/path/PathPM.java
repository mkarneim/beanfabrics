...
public class PathPM extends TextPM {
	...
	@Validation(message="This path points into the void") @SortOrder(2)
	boolean isComplete() {
		Path path = new Path( this.getText());		
		return ( this.root==null || this.root.getPathInfo(path)!=null);
	}
	
	@Validation(message="The object at the end of this path does not match the required type") @SortOrder(3)
	boolean isCorrect() {
		Path path = new Path( this.getText());
