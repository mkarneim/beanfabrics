...
public class PathBrowserPM extends AbstractPM {
	private static final URL SUCCESS_ICON = PathBrowserPM.class
			.getResource("agt_action_success.png");
	private static final URL FAIL_ICON = PathBrowserPM.class
			.getResource("agt_action_fail.png");
	private static final URL WARNING_ICON = PathBrowserPM.class
			.getResource("messagebox_warning.png");
	...
	void updateStatus() {
		if (this.currentPath.isEmpty()) {
			...
			this.status.setText(vState == null ? null : "<html>"
					+ vState.getMessage() + "</html>");
	...
	ValidationState matchesRequiredModelInfo() {
		if (this.requiredModelInfo == null) {
			...
			boolean valid = (currentModelInfo != null && this.requiredModelInfo
					.isAssignableFrom(currentModelInfo));
			return valid ? null : new ValidationState(
					"This path is invalid because it points to an invalid PresentationModel type. "
							+ "\nRequired is "
							+ this.requiredModelInfo.getName());
		}
	}
