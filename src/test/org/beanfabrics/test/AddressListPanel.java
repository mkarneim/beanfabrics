...
public class AddressListPanel extends JPanel implements View<AddressListPM>, ModelSubscriber {
	...
	private BnTable getBnTable() {
		if (bnTable == null) {
			...
			bnTable.setColumns(new BnColumn[] { new BnColumn(new Path("appartment"), "Appartment"), new BnColumn(new Path("street"), "Street"), new BnColumn(new Path("city"), "City"), new BnColumn(new Path("zip"), "ZIP"), new BnColumn(new Path("country"), "Country") });
		}
		return bnTable;
	}
