...
public class PathEvaluation implements Iterable<PathEvaluation.Entry> {
	...
	public PathEvaluation.Entry getResult() {
		if (this.resolvedCompletely == false) {
			throw new IllegalStateException(
					"could not evaluate this path completely. Resolved '"
					+this.path.getSubPath(0, this.resolvedLength)+"'");
		} else {
			return getLastEntry();
		}
	}
	 ...
	 *
	 */
	public static PresentationModel evaluate(PresentationModel root, Path path) throws IllegalArgumentException, EvaluationException {
	 ...
	 *
	 */
	public static PresentationModel evaluateOrNull(PresentationModel root, Path path) throws IllegalArgumentException {
		PathEvaluation eval = new PathEvaluation(root, path);
		if ( eval.isCompletelyResolved()) {
			return eval.getResult().value;
		} else {
