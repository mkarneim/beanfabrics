...
//	private Color borderColor = new Color(107, 136, 177);
	...
	public PathPainter() {
		Font basefont = UIManager.getLookAndFeelDefaults()
				.getFont("Label.font");
	...
	public void paintPath(Graphics g, int x, int y, Path path, JComponent c) {
		if (path != null) {
			...
			String clippedText = layoutCL(c, fm,
					text, textR, textR);
	...
	private String layoutCL(JComponent c, FontMetrics fontMetrics, String text,
			Rectangle viewR, Rectangle textR) {
		Rectangle iconR = emptyR;
		return SwingUtilities.layoutCompoundLabel(c, fontMetrics, text, /* icon */
				null, verticalAlignment, horizontalAlignment,
