...
public class DefaultContext implements Context {
	...
	public DefaultContext() {
		...
		parentListener = new ContextListener() {
			public void serviceRemoved(ServiceRemovedEvent evt) {
				removeServiceEntry(evt.getServiceEntry().getOrigin(), evt
						.getServiceEntry().getType());
			}

			public void serviceAdded(ServiceAddedEvent evt) {
				ServiceEntry entry = evt.getServiceEntry();
				addServiceEntry(new ServiceEntry(entry.getDistance() + 1, entry
						.getOrigin(), entry.getService(), entry.getType()));
			}
		...
		unmodifiableServiceEntries = Collections
				.unmodifiableList(serviceEntries);
	}
	...
	private boolean addServiceEntry(ServiceEntry entry) {
		if (LOG.isDebugEnabled()) {
			LOG.debug("adding service '" + entry.getType().getName() + "' to "
					+ this);
		}
	...
	protected Object removeServiceEntry(Context origin, Class type) {
		...
		while (it.hasNext()) {
			...
			if (entry.getType().equals(type) && entry.getOrigin() == origin) {
				if (LOG.isDebugEnabled()) {
					LOG.debug("removing service '" + entry.getType().getName()
							+ "' from " + this);
				}
				it.remove();
	...
	public ServiceEntry findService(Class type) {
		...
		for (ServiceEntry entry : serviceEntries) {
			if (type.equals(entry.getType())) {
				if (result == null
						|| result.getDistance() > entry.getDistance()) {
					result = entry;
				}
			}
		}
